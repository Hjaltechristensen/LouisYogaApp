...
public partial class OneMinTimer : ContentPage
{
    bool startStop = true;
    bool loopBool = true;
    ...
    private async void UpdateArc()
    {
        ...
        while (loopBool)
        {
            for (i = 0; i < RoundChoiceOneMinTimer.tal; i++)
            {
                if (loopBool && i != 0 && i < RoundChoiceOneMinTimer.tal && checkNewRound)
                {
                    runde++;
                }
                rounds.Text = $"Round {runde}/{RoundChoiceOneMinTimer.tal}";
                while (startStop)
                {
                    ...
                    if (runde == RoundChoiceOneMinTimer.tal)
                    {
    ...
    private void stop_Clicked(object sender, EventArgs e)
    {
        ...
        genstartSek = sekTal;
        timer.Text = $"0{genstartMin}:0{genstartSek}";
        rounds.Text = $"Round {1}/{RoundChoiceOneMinTimer.tal}";
        runde = 1;
        i = 0;
    }
